...
class SmartDocumentAnalyzer:
    ...
    def analyze_document_structure(self, pages_df: pl.DataFrame) -> Dict[str, Any]:
        ...
        content_analysis = analysis_df.select(
            [
                pl.when(pl.col("char_count") < 50)
                .then(pl.lit("minimal"))
                .when(pl.col("char_count") < 500)
                .then(pl.lit("short"))
                .when(pl.col("char_count") < 2000)
                .then(pl.lit("medium"))
                .otherwise(pl.lit("long"))
                .alias("content_length"),
                pl.when(pl.col("likely_header"))
                .then(pl.lit("header"))
                .when(pl.col("numbered_item"))
                .then(pl.lit("numbered_list"))
                .when(pl.col("bullet_point"))
                .then(pl.lit("bullet_list"))
                .when(pl.col("contains_numbers"))
                .then(pl.lit("data_rich"))
        ...
        page_types = (
            content_analysis.group_by("content_type").len().sort("len", descending=True)
        )
        length_distribution = (
            content_analysis.group_by("content_length")
            .len()
