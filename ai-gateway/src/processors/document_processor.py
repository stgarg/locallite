...
class DocumentProcessor:
    ...
    def batch_process(self, file_paths: List[Union[str, Path]], **kwargs) -> Dict[str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            for file_path, result in results.items():
                ...
                row = {
                    ...
                    'format_type': result.get('processing_metadata', {}).get('detected_format', 'unknown'),
                    'converter': result.get('processing_metadata', {}).get('converter_used', 'unknown')
                }
                ...
                if stats:
                    row.update({
                        ...
                        'total_pages': stats.get('total_pages', stats.get('total_elements', stats.get('total_words', 0))),
            ...
            summary = {
                ...
                'success_rate': (df.filter(pl.col('success')).height / len(results) * 100) if results else 0,
                'format_distribution': df.group_by('format_type').len().to_dict(as_series=False) if df.height > 0 else {},
                'converter_usage': df.group_by('converter').len().to_dict(as_series=False) if df.height > 0 else {},
            }
            
            # Add character statistics if available
            if 'total_characters' in df.columns:
                char_stats = df.filter(pl.col('success')).select(pl.col('total_characters'))
