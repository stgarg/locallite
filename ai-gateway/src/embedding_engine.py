...
class OptimizedEmbeddingEngine:
    ...
    def _load_model_assets(self):
        ...
        if os.path.exists(config_path):
            with open(config_path, 'r') as f:
                ...
                logger.info(f"Loaded model config: {self.config.get('model_type', 'unknown')}")
    ...
    def _select_optimal_provider(self, batch_size: int) -> Tuple[ort.InferenceSession, str]:
        """
    ...
    def encode(self, texts: List[str]) -> Tuple[np.ndarray, Dict[str, any]]:
        ...
        logger.debug(f"Generated {len(embeddings)} embeddings using {provider} "
    ...
    def get_model_info(self) -> Dict[str, any]:
        ...
        return {
            ...
            "providers": {
                ...
                "cpu_providers": self.session_cpu.get_providers() if self.session_cpu else None,
                "npu_providers": self.session_npu.get_providers() if self.session_npu else None
            },
            "performance_rules": {
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i, texts in enumerate(test_cases, 1):
            ...
            print(f"Test {i}: {len(texts)} texts → {perf['provider']} "
