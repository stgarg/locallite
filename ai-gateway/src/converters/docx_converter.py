    ...
    def _combine_element_text(self, elements_data: List[Dict]) -> str:
        ...
        for element in elements_data:
            if element.get("text") and element["has_text"]:
                ...
                texts.append(
                    f"--- {element_type.title()} {element_num} ---\n{element['text']}\n"
                )
        return "\n".join(texts)
    ...
    def analyze_structure(self, file_path: Path) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "text_statistics": {
                ...
                "average_chars_per_element": df.select(
                    pl.col("char_count").mean()
                ).item(),
        ...
        if "style" in df.columns:
            try:
                style_counts = (
                    df.filter(pl.col("element_type") == "paragraph")
                    .select("style")
                    .value_counts()
                )
        ...
        if analysis["text_statistics"]["average_chars_per_element"] < 50:
            issues.append(
                "Very short average element length - document may be poorly structured"
            )
