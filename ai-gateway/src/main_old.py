...
class EmbeddingRequest(BaseModel):
    ...
    model: str = Field(
        default="bge-small-en-v1.5", description="Model to use for embedding"
    )
    encoding_format: str = Field(default="float", description="Encoding format")
    dimensions: Optional[int] = Field(
        None, description="Number of dimensions (optional)"
    )
...
async def create_chat_completion(
    ...
    try:
        ...
        logger.info(
            f"Chat completion generated in {processing_time:.3f}s using {request.model}"
        )
